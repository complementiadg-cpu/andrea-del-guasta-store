...
export default function Checkout() {
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Salva l'ordine in stato pending su Supabase
      ...
      if (dbError || !ordine) {
        throw new Error(dbError?.message || "Impossibile salvare l'ordine nel database.");
      }

      // 2. Unica chiamata alla Edge Function tramite il wrapper createStripeCheckout
      const checkoutUrl = await createStripeCheckout({
        items: cartItems,
        shippingCost: 0,
        shippingLabel: "Spedizione Standard",
        email: customerDetails.email,
        orderId: ordine.id,
      });

      // 3. Reindirizzamento alla pagina di checkout sicura di Stripe
      ...
      toast({
        ...
        description: err.message || "Si è verificato un errore di connessione con il server.",
      });
    } finally {
